...
setInterval(function() {
    ...
    browser.storage.local.get("doubloonsPerHour").then((result) => {
        ...
        elements.forEach(async function(element) {
            ...
            if (buttons.length === 0 || buttons[0].innerHTML.includes("soon") || buttons[0].innerHTML.includes("stock")) {
                return;
            }
...
setInterval(async function() {
    ...
    let doubloonsUntil = parseInt(doubText.innerText.split("doubloons")[0].trim());
    const doubloonsPerHour = (await browser.storage.local.get("doubloonsPerHour")).doubloonsPerHour;
    const timeUntil = doubloonsUntil / doubloonsPerHour;
    console.log(doubText.innerHTML.replace("doubloons", "Doubloons (" + timeUntil.toFixed(2) + " hours)"))
    doubText.innerHTML = doubText.innerHTML.replace("doubloons", "Doubloons (" + timeUntil.toFixed(2) + " hours)");
